...
# 
# ggl_spend <- readr::read_csv("data/NZ.csv")

# ggl_spend <- ggl_spend %>% rename(Advertiser_ID = ggl_id) %>% left_join(gglstats %>% distinct(Advertiser_ID, Advertiser_Name)) %>% arrange(Advertiser_ID) 
...
if(!exists("daily_spending_old")){
  daily_spending_old <- tibble(advertiser_id = "NA", date = NA)
}
...
saveRDS(daily_spending %>% bind_rows(daily_spending_old) %>% distinct() %>% filter(advertiser_id != "NA"), file = "data/ggl_daily_spending.rds")
...
if (!exists("ggl_sel_sp_old")) {
  ggl_sel_sp_old <- tibble(from = as.Date("2020-01-01"))
}
